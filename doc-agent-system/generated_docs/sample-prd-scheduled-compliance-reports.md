...
3. The system will generate and deliver the report at the specified time, with a maximum generation time of 15 minutes and a delivery time of 30 minutes.
4. If a scheduled report fails, the system will retry twice with delays of 5 minutes and 15 minutes before notifying the template owner.
...
3. Choose whether to attach the report directly or send a secure download link (which expires after 7 days).
4. Save your changes to enable email delivery for the scheduled report.
...
All generated reports are stored as immutable artifacts with metadata that includes parameters, generation time, row count, and delivery status. These artifacts follow a dedicated retention policy, defaulting to 7 years, independent of message retention policies. Every action related to report generation, download, and delivery is logged in an immutable audit trail.
...
A: The system will retry the report generation twice with exponential backoff (5 minutes, then 15 minutes) and notify the template owner if all retries fail.
