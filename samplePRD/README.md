...
The PRD acceptance-test contract is also supported through compatibility routes: `POST /templates`, `GET /templates/{id}`, `POST /schedules`, `POST /date-ranges/resolve`, synchronous `POST /reports/generate`, `GET /artifacts/{id}`, and `GET /audit-logs`. Production clients should prefer the authenticated `/api` routes.
